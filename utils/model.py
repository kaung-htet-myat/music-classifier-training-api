...
def build_model(data_cfg: DictConfig, model_cfg: DictConfig, device):
    ...
    if data_cfg.preprocessing.name not in ["spectrogram", "melspectrogram", "mfcc"]:
        raise UnsupportedParameterError(
            'Data preprocessing should be one of "spectrogram", "melspectrogram" or "mfcc"'
        )

    if not model_cfg.backbone == "efficientnet":
        raise UnsupportedParameterError(
            'Only "efficientnet" is provided as backbone for now'
        )

    if model_cfg.name not in ["linear", "prcnn", "vit"]:
        raise UnsupportedParameterError(
            'Model architecture should be one of "linear", "prcnn" or "vit"'
        )
    ...
    if data_cfg.preprocessing.name == "melspectrogram":
        input_width = int(
            (
                ...
                - data_cfg.preprocessing.fft_size
            )
            // data_cfg.preprocessing.hop_length
            + 1
        )
    ...
    if model_cfg.name == "prcnn":
        model = PRCNN(
            ...
            backbone,
            (
                model_cfg.backbone_out_channel,
                model_cfg.backbone_out_height,
                model_cfg.backbone_out_width,
            ),
