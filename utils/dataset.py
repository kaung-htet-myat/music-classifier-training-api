...
def get_transforms(data_cfg: DictConfig):
    """
    Dataset transform/augment functions.
    Args:
        data_cfg (DictConfig): hydra data config object
    Return:
        train_transform (Callable): train transform callback function
        test_transform (Callable): test transform callback function
    """
    ...
    def train_transform(features):
        ...
        resampled = nn.functional.pad(
            ...
            (
                0,
                data_cfg.preprocessing.sample_rate
                * data_cfg.preprocessing.sample_length
                - waveform.size()[-1],
            ),
            "constant",
            0.0,
...
def get_dataset(tfrec_pattern, index_pattern, records, transform, buffer_size):
    ...
    Args:
        tfrec_pattern (str): tfrecord pattern for .tfrecords files
        index_pattern (str): index pattern for .index files
        records (list): list of tfrecords files
        transform (Callable): dataset transform/augment callback function
        buffer_size (int): shuffle buffer size
    Return:
        dataset: a MultiTFRecordDataset object
    """
    ...
    if len(records) == 0:
        raise ParameterNotProvidedError(
            "train, dev or test record list cannot be empty"
        )
    ...
    dataset = MultiTFRecordDataset(
        ...
        transform=transform,
        infinite=False,
        shuffle_queue_size=buffer_size,
    )
