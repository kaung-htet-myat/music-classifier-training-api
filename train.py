...
from utils.training import get_optimizer, get_scheduler, train_epoch, test_epoch
...
def main(cfg: DictConfig):
    ...
    dev_dataset = get_dataset(
        tfrec_pattern,
        index_pattern,
        data_cfg.dev_records,
        test_transform,
        training_cfg.buffer_size,
    )
    dev_loader = torch.utils.data.DataLoader(
        dev_dataset, batch_size=training_cfg.batch_size
    )
    ...
    if training_cfg.load and not training_cfg.initial_epoch == 0:
        load_path = os.path.join(
            ...
            f"{exp_name}_epoch_{training_cfg.initial_epoch-1}.pth",
        )

        if not os.path.exists(load_path):
            raise UnsupportedParameterError(
                "model checkpoint path to load does not exist"
            )
