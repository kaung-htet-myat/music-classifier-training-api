...
class PRCNN(nn.Module):
    def __init__(
        ...
        self.feature_extractor = create_feature_extractor(
            backbone_model, return_nodes, suppress_diff_warning=True
        )
        # self.bn_1 = nn.BatchNorm2d(backbone_out_dims[0])
        self.dropout_1 = nn.Dropout(0.2)
        self.avg_pool_1 = nn.AvgPool2d(
            kernel_size=(backbone_out_dims[1], backbone_out_dims[2])
        )
        self.maxpool_1 = nn.MaxPool2d((2, 1), stride=(2, 1))
        self.bi_rnn = nn.LSTM(
            int(input_dims[1] / 2), hidden_dims_1, bidirectional=True, batch_first=True
        )
        self.dropout_2 = nn.Dropout(0.2)
        self.linear_1 = nn.Linear(
            int(hidden_dims_1 + backbone_out_dims[0]), hidden_dims_2
        )
        self.relu_1 = nn.ReLU()
    ...
    def forward(self, x):
        ...
        rnn_feat = h_n[0]
        rnn_feat = rnn_feat[0] + rnn_feat[1]
